...
//     // }
// 	if($type) {
// 		$page = "/pages/index/index?gotourl=/pagesB/single/single&id=".$post_id."&isshare=1&posttype=".$type."&pagetype=single";
// 	} else {
// 		$page = "/pages/index/index";
//     }
//     $data = array(
...
// 	// }
// 	if($post_type) {
// 		$page = "/pages/index/index?gotourl=/pagesB/single/single&id=".$post_id."&isshare=1&posttype=".$post_type."&pagetype=single";
// 	} else {
// 		$page = "/pages/index/index";
//     }
// 	$parents = get_comment( $parent_id );
...
if( defined('MINI_PROGRAM_API_PLUGIN')&&wp_miniprogram_option('update_tpl_id')!='') {
	add_action('publish_post', 'wp_bee_post_update_notice_event');
	add_action('publish_beebee_topic', 'wp_bee_post_update_notice_event');
	add_action('publish_beebee_library', 'wp_bee_post_update_notice_event');
	add_action('publish_beebee_quot', 'wp_bee_post_update_notice_event');
	add_action('publish_lesson', 'wp_bee_post_update_notice_event');
	add_action('publish_to_publish', function () {
		remove_action('publish_post', 'wp_bee_post_update_notice_event');
		remove_action('publish_beebee_library', 'wp_bee_post_update_notice_event');
		remove_action('publish_beebee_topic', 'wp_bee_post_update_notice_event');
		remove_action('publish_beebee_quot', 'wp_bee_post_update_notice_event');
		remove_action('publish_beebee_quot', 'wp_bee_post_update_notice_event');
	},11,1);
}


function wp_bee_post_update_notice_event( $post_id ) {
    if( ! wp_next_scheduled( 'wp_bee_post_wechat_notice_event', array( "post_id" => $post_id ) ) ) {
        wp_schedule_single_event( time() + 60, 'wp_bee_post_wechat_notice_event', array( "post_id" => $post_id ) ); // 定时计划延时 3 分钟执行
	}
	// if( wp_miniprogram_option('qq_appid') && wp_miniprogram_option('qq_secret') ) {
...
add_action( 'wp_bee_post_wechat_notice_event', 'we_bee_miniprogram_posts_update_notice' );
// add_action( 'wp_bee_post_tencent_notice_event', 'mp_bee_qq_subscribe_update_posts_notice' );

function we_bee_miniprogram_posts_update_notice( $post_id ) {
    ...
    // $page = "/pages/detail/detail?id=".$post_id;
	if($type=='post') {
		// $page = "/pagesB/single/single?id=".$post_id."&isshare=1&posttype=posts&pagetype=single";
		$page = "/pages/index/index?gotourl=/pagesB/single/single&id=".$post_id."&isshare=1&posttype=posts&pagetype=single";
	}else if($type) {
		// $page = "/pages/index/index?page/pagesB/single/single?id=".$post_id."&isshare=1&posttype=".$type."&pagetype=single";
		$page = "/pages/index/index?gotourl=/pagesB/single/single&id=".$post_id."&isshare=1&posttype=".$type."&pagetype=single";
	} else {
		$page = "/pages/index/index";
    }
    ...
    $data = array(
        "thing1"	=> array( "value" => '内容更新' ),
		"thing2"	=> array( "value" => html_entity_decode( $title ) ),
		"thing3"	=> array( "value" => html_entity_decode( $content?$content:'点击查看内容' ) )
    );
    foreach( $subscriber as $subscribe ) {
        ...
        $status = we_bee_miniprogram_subscribe_message_action( $task_id, $contents );
		sleep( 120 ); 
    }
}

function we_bee_miniprogram_subscribe_message_action( $task_id, $contents ) {
