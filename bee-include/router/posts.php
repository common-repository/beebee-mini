...
class WP_REST_BEE_Posts_Router extends WP_REST_Controller {
	...
	public function get_sticky_posts( $request ) {
		...
		// }
		$args = array( 'posts_per_page' => $per_page, 'offset' => $offset, 'post_type' => $post_type,'orderby' => 'date', 'meta_key' => 'bee_focus',
		'meta_query' => array(
			array(
			 'key'  => 'bee_focus', 
			 'value' => true, 
	...
	public function get_rand_posts( $request ) {
		...
		$per_page = isset($request["per_page"])?$request["per_page"]:10;
		$offset = ($page * $per_page) - $per_page;
		$args = array( 'posts_per_page' => $per_page, 'offset' => $offset,'post_type' => $post_type, 'orderby' => 'rand', 'date_query' => array( array( 'after' => '1 year ago' ) ) );
		$args = apply_filters( "rest_post_query", $args, $request );
		$query  = new WP_Query();
		$posts = $query->query( $args );
		// foreach ( $posts as $post ) {
	...
	public function get_most_posts( $request ) {
		...
		$args = array( 'posts_per_page' => $per_page, 'offset' => $offset, 'post_type' => $post_type,'meta_key' => $meta, 'orderby' => 'meta_value_num', 'order' => 'DESC', 'update_post_meta_cache' => false, 'cache_results' => false );
		$args = apply_filters( "rest_post_query", $args, $request );
		$query  = new WP_Query();
		$posts = $query->query( $args );
		// foreach ( $posts as $post ) {
	...
	public function get_relate_posts( $request ) {
		...
		if( $post_id ) {
			...
			$args = array( 'posts_per_page' => $per_page, 'offset' => $offset, 'orderby' => 'rand', 'order' => 'DESC', 'post_type' => $post_type,'post__not_in' => array( $post_id ), 'tag__in' => $post_tag );
		}
		$query  = new WP_Query();
		$posts = $query->query( $args );
		if($posts) {
