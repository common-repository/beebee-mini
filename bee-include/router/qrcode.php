...
class WP_REST_BEE_Qrcode_Router extends WP_REST_Controller {
	...
	public function wp_post_qrcode( $request ) {
		$post_id = $request['id'];
		// $post_pagetype = isset($request['pagetype']) ? $request['pagetype'] : get_post_type( $post_id );
		$qrcode_type = isset($request['type']) ? $request['type'] : '';
		if( $post_id ) {
			$post_type = get_post_type( $post_id );
		} else {
		...
		if( $post_type != 'custom' ) {
			$post_path = '/pages/index/index?gotourl=/pagesB/single/single&id='.$post_id.'&isshare=1&posttype='.$post_type.'&pagetype=single'; //小鱼哥 修改了二维码路径
		} else {
			$post_path = "/pages/index/index";
		}
		$path = isset($request['path']) && $request['path'] ? $request['path'] : $post_path;
