...
function my_edit_beebee_library_columns($columns)
{
    $columns = array(
        ...
        'title' => esc_html(__('书名')),
        'id' => esc_html(__('ID')),
        'thumbimage' => esc_html(__('封面')),
        'desc' => esc_html(__('简述')),
        'beebee_library_state' => esc_html(__('状态')),
...
function my_manage_beebee_library_columns($column, $post_id)
{
    ...
    switch ($column) {
        ...
        case 'thumbimage':
            ...
            esc_html(printf(__('<div style="width:45px;height:63px;border-radius: 3px;background-image:url(%s);background-size: cover;background-repeat: no-repeat;background-position: center;"></div>'), $thumbimage));
            break;
